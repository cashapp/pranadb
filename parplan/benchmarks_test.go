...
func BenchmarkParser(b *testing.B) {
	for i := 0; i < b.N; i++ {
		parser := NewParser()
		stmt, _, err := parser.Parse("select col0, col1, col2 from table1 where col1 = 123456 order by col0, col1")
...
func BenchmarkPreprocess(b *testing.B) {
	schema := createTestSchema()
	planner := NewPlanner(schema)
	ast, _, err := planner.parser.Parse("select col0, col1, col2 from table1 where col1 = 123456 order by col0, col1")
	require.NoError(b, err)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
