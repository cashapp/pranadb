...
func (d *Dragon) BatchWrittenLocally(shardID uint64, batchID []byte) (bool, error) {
	d.lock.RLock()
	defer d.lock.RUnlock()
	sm, ok := d.shardSMs[shardID]
	if !ok {
		return false, errors.Errorf("cannot find state machine for shard %d", shardID)
	}
	return sm.HasPersistedBatchLocally(batchID), nil
}
