...
func ParseBinaryFromString(s string) (bj BinaryJSON, err error) {
	if len(s) == 0 {
		err = tidb.ErrInvalidJSONText.GenWithStackByArgs("The document is empty")
		return
	}
	data := hack.Slice(s)
	if !json.Valid(data) {
		err = tidb.ErrInvalidJSONText.GenWithStackByArgs("The document root must not be followed by other values.")
		return
	}
	if err = bj.UnmarshalJSON(data); err != nil && !tidb.ErrJSONObjectKeyTooLong.Equal(err) {
		err = tidb.ErrInvalidJSONText.GenWithStackByArgs(err)
	}
	return
}
...
func appendBinaryObject(buf []byte, x map[string]interface{}) ([]byte, error) {
	...
	for i, field := range fields {
		...
		if keyLen > math.MaxUint16 {
			return nil, tidb.ErrJSONObjectKeyTooLong.GenWithStackByArgs("JSON object key too long")
		}
