...
func StrToInt(sc *stmtctx.StatementContext, str string, isFuncCast bool) (int64, error) {
	...
	if err1 != nil {
		return iVal, tidb.ErrOverflow.GenWithStackByArgs("BIGINT", validPrefix)
	}
...
func StrToUint(sc *stmtctx.StatementContext, str string, isFuncCast bool) (uint64, error) {
	...
	if err1 != nil {
		return uVal, tidb.ErrOverflow.GenWithStackByArgs("BIGINT UNSIGNED", validPrefix)
	}
...
func NumberToDuration(number int64, fsp int8) (Duration, error) {
	if number > TimeMaxValue {
		...
		return dur, tidb.ErrOverflow.GenWithStackByArgs("Duration", strconv.Itoa(int(number)))
	...
	if number/10000 > TimeMaxHour || number%100 >= 60 || (number/100)%100 >= 60 {
		return ZeroDuration, errors.Trace(tidb.ErrTruncatedWrongVal.GenWithStackByArgs(TimeStr, strconv.FormatInt(number, 10)))
	}
...
func getValidIntPrefix(sc *stmtctx.StatementContext, str string, isFuncCast bool) (string, error) {
	...
	if validLen == 0 || validLen != len(str) {
		return valid, errors.Trace(sc.HandleTruncate(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("INTEGER", str)))
	}
	return valid, nil
}
...
func floatStrToIntStr(sc *stmtctx.StatementContext, validFloat string, oriStr string) (intStr string, _ error) {
	...
	if exp >= 0 && (intCnt > 21 || intCnt < 0) {
		...
		sc.AppendWarning(tidb.ErrOverflow.GenWithStackByArgs("BIGINT", oriStr))
	...
	if err1 != nil {
		if err2, ok := err1.(*strconv.NumError); ok {
			...
			if err2.Err == strconv.ErrRange {
				err1 = sc.HandleTruncate(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("DOUBLE", str))
				if math.IsInf(f, 1) {
					f = math.MaxFloat64
...
func ConvertJSONToInt(sc *stmtctx.StatementContext, j json.BinaryJSON, unsigned bool, tp byte) (int64, error) {
	...
	case json.TypeCodeObject, json.TypeCodeArray:
		return 0, sc.HandleTruncate(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("INTEGER", j.String()))
	...
	return 0, errors.Error("Unknown type code in JSON")
}
...
func ConvertJSONToFloat(sc *stmtctx.StatementContext, j json.BinaryJSON) (float64, error) {
	switch j.TypeCode {
	case json.TypeCodeObject, json.TypeCodeArray:
		return 0, sc.HandleTruncate(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("FLOAT", j.String()))
	case json.TypeCodeLiteral:
		switch j.Value[0] {
		case json.LiteralNil, json.LiteralFalse:
			return 0, nil
...
func ConvertJSONToDecimal(sc *stmtctx.StatementContext, j json.BinaryJSON) (*MyDecimal, error) {
	...
	switch j.TypeCode {
	case json.TypeCodeObject, json.TypeCodeArray:
		err = tidb.ErrTruncatedWrongVal.GenWithStackByArgs("DECIMAL", j.String())
	case json.TypeCodeLiteral:
		switch j.Value[0] {
		case json.LiteralNil, json.LiteralFalse:
			res = res.FromInt(0)
...
func getValidFloatPrefix(sc *stmtctx.StatementContext, s string, isFuncCast bool) (valid string, err error) {
	...
	if validLen == 0 || validLen != len(s) {
		err = errors.Trace(sc.HandleTruncate(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("FLOAT", s)))
	}
	return valid, err
}
