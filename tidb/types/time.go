...
import (
	...
	"github.com/squareup/pranadb/tidb/util/parser"
	"math"
	"regexp"
	"strconv"
	"strings"
	...
	tidbMath "github.com/squareup/pranadb/tidb/util/math"
	//"github.com/squareup/pranadb/tidb/util/parser"
)
...
func (t Time) RoundFrac(sc *stmtctx.StatementContext, fsp int8) (Time, error) {
	...
	if t1, err := t.GoTime(sc.TimeZone); err == nil {
		...
		if t2.Day()-1 > 0 {
			return t, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, t.String()))
		}
		var ok bool
...
func parseDatetime(sc *stmtctx.StatementContext, str string, fsp int8, isFloat bool) (Time, error) {
	...
	*/
	...
	switch len(seps) {
	case 0:
		return ZeroDatetime, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, str))
	case 1:
		l := len(seps[0])
		...
		case 6, 5:
			...
			year = adjustYear(year)
		default:
			return ZeroDatetime, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, str))
		}
		if l == 5 || l == 6 || l == 8 {
			// YYMMDD or YYYYMMDD
		...
		if truncatedOrIncorrect && sc != nil {
			sc.AppendWarning(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("datetime", str))
			err = nil
		}
	case 2:
		return ZeroDatetime, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, str))
	...
	if hasTZ {
		...
		if !hhmmss {
			return ZeroDatetime, errors.Trace(tidb.ErrWrongValue.GenWithStack(DateTimeStr, str))
		}
		if len(tzHour) != 0 {
...
func scanTimeArgs(seps []string, args ...*int) error {
	if len(seps) != len(args) {
		return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, seps))
	}
...
func AdjustYear(y int64, adjustZero bool) (int64, error) {
	...
	if y < 0 {
		return 0, errors.Trace(tidb.ErrInvalidYear)
	}
	if y < int64(MinYear) {
		return int64(MinYear), errors.Trace(tidb.ErrInvalidYear)
	}
	if y > int64(MaxYear) {
		return int64(MaxYear), errors.Trace(tidb.ErrInvalidYear)
	}
...
func matchDuration(str string, fsp int8) (Duration, error) {
	...
	if len(str) == 0 {
		return ZeroDuration, tidb.ErrTruncatedWrongVal.GenWithStackByArgs("time", str)
	}
	...
	if hhmmss[0] > TimeMaxHour {
		...
		return Duration{t, fsp}, tidb.ErrTruncatedWrongVal.GenWithStackByArgs("time", str)
	}
...
func ParseDuration(sc *stmtctx.StatementContext, str string, fsp int8) (Duration, error) {
	...
	if !canFallbackToDateTime(rest) {
		return d, tidb.ErrTruncatedWrongVal.GenWithStackByArgs("time", str)
	}
...
func TruncateOverflowMySQLTime(d gotime.Duration) (gotime.Duration, error) {
	if d > MaxTime {
		return MaxTime, tidb.ErrTruncatedWrongVal.GenWithStackByArgs("time", d)
	} else if d < MinTime {
		return MinTime, tidb.ErrTruncatedWrongVal.GenWithStackByArgs("time", d)
	}
...
func getTime(sc *stmtctx.StatementContext, num, originNum int64, tp byte) (Time, error) {
	...
	if !ok {
		...
		return ZeroDatetime, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, numStr))
	}
...
func parseDateTimeFromNum(sc *stmtctx.StatementContext, num int64) (Time, error) {
	...
	// Check MMDD.
	if num < 101 {
		return t, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, strconv.FormatInt(num, 10)))
	}
...
func checkDateType(t CoreTime, allowZeroInDate, allowInvalidDate bool) error {
	...
	if !allowZeroInDate && (month == 0 || day == 0) {
		return tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, fmt.Sprintf("%04d-%02d-%02d", year, month, day))
	}
...
func checkDateRange(t CoreTime) error {
	...
	if t.Year() < 0 || t.Month() < 0 || t.Day() < 0 {
		return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, t))
	}
...
func checkMonthDay(year, month, day int, allowInvalidDate bool) error {
	if month < 0 || month > 12 {
		return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, fmt.Sprintf("%d-%d-%d", year, month, day)))
	}
...
func checkTimestampType(sc *stmtctx.StatementContext, t CoreTime) error {
	...
	if sc == nil {
		return errors.Error("statementContext is required during checkTimestampType")
	}
...
func checkDatetimeType(t CoreTime, allowZeroInDate, allowInvalidDate bool) error {
	...
	if hour < 0 || hour >= 24 {
		return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, strconv.Itoa(hour)))
	}
	if minute < 0 || minute >= 60 {
		return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, strconv.Itoa(minute)))
	}
	if second < 0 || second >= 60 {
		return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, strconv.Itoa(second)))
	}
...
func parseSingleTimeValue(unit string, format string, strictCheck bool) (int64, int64, int64, int64, error) {
	...
	if err != nil {
		return 0, 0, 0, 0, tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, format)
	}
	...
	if decimalPointPos < lf {
		...
		if unit != "SECOND" {
			err = tidb.ErrTruncatedWrongVal.GenWithStackByArgs(format)
		}
		dv *= sign
	}
	switch strings.ToUpper(unit) {
	case "MICROSECOND":
		if strictCheck && tidbMath.Abs(riv) > TimeMaxValueSeconds*1000 {
			return 0, 0, 0, 0, tidb.ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
		}
...
func parseTimeValue(format string, index, cnt int) (int64, int64, int64, int64, error) {
	...
	if len(matches) > cnt {
		return 0, 0, 0, 0, tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, originalFmt)
	}
	for i := range matches {
		if neg {
...
func parseAndValidateDurationValue(format string, index, cnt int) (int64, error) {
	...
	if year != 0 || month != 0 || tidbMath.Abs(day) > TimeMaxHour/24 {
		return 0, tidb.ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
	}
...
func ExtractDurationValue(unit string, format string) (Duration, error) {
	...
	case "YEAR_MONTH":
		...
		return ZeroDuration, tidb.ErrDatetimeFunctionOverflow.GenWithStackByArgs("time")
	default:
...
func (t Time) convertDateFormat(b rune, buf *bytes.Buffer) error {
	...
	case 'b':
		m := t.Month()
		if m == 0 || m > 12 {
			return errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, strconv.Itoa(m)))
		}
...
func (t *Time) StrToDate(sc *stmtctx.StatementContext, date, format string) bool {
	...
	if warning {
		...
		sc.AppendWarning(tidb.ErrTruncatedWrongVal.GenWithStackByArgs(DateTimeStr, date))
	}
	return true
}
...
func mysqlTimeFix(t *CoreTime, ctx map[string]int) error {
	...
	if valueAMorPm, ok := ctx["%p"]; ok {
		if _, ok := ctx["%H"]; ok {
			return tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, t)
		}
		if t.Hour() == 0 {
			return tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, t)
		}
