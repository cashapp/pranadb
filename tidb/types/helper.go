...
func TruncateFloat(f float64, flen int, decimal int) (float64, error) {
	if math.IsNaN(f) {
		// nan returns 0
		return 0, tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", "")
	}
	...
	if f > maxF {
		f = maxF
		err = tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", "")
	} else if f < -maxF {
		f = -maxF
		err = tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", "")
	}
...
func strToInt(str string) (int64, error) {
	...
	for ; i < len(str); i++ {
		...
		if r >= uintCutOff {
			r = 0
			err = errors.Trace(tidb.ErrBadNumber)
			break
		}
		r = r * uint64(10)
	...
	if !negative && r >= intCutOff {
		return math.MaxInt64, errors.Trace(tidb.ErrBadNumber)
	}

	if negative && r > intCutOff {
		return math.MinInt64, errors.Trace(tidb.ErrBadNumber)
	}
