		...
		if len(overflowed) != 0 {
			...
			if len(trimed) == 0 && !IsBinaryStr(tp) && IsTypeChar(tp.Tp) {
				if tp.Tp == mysql.TypeVarchar {
					sc.AppendWarning(tidb.ErrTruncated.GenWithStack("Data truncated, field len %d, data len %d", flen, characterLen))
				}
			} else {
				err = tidb.ErrDataTooLong.GenWithStack("Data Too Long, field len %d, data len %d", flen, characterLen)
			}
		}
...
func (d *Datum) convertToUint(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	case KindString, KindBytes:
		...
		if err1 != nil && tidb.ErrOverflow.Equal(err1) && !sc.ShouldIgnoreOverflowError() {
...
func (d *Datum) convertToMysqlTimestamp(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	case KindMysqlTime:
		...
		if sc.InReorgAttribute {
			...
			if err != nil {
				ret.SetMysqlTime(t)
				return ret, errors.Trace(tidb.ErrWrongValue.GenWithStackByArgs(DateTimeStr, t.String()))
			}
		} else {
			t = d.GetMysqlTime()
		}
...
func (d *Datum) convertToMysqlTime(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	case KindUint64:
		...
		if intOverflow64 {
			...
			t, err = ZeroDate, tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, uNum)
		} else {
			t, err = ParseTimeFromNum(sc, d.GetInt64(), tp, fsp)
		}
	case KindMysqlJSON:
...
func (d *Datum) convertToMysqlDuration(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	case KindInt64, KindFloat32, KindFloat64, KindMysqlDecimal:
		...
		if timeNum > MaxDuration && timeNum < 10000000000 {
			...
			return ret, tidb.ErrWrongValue.GenWithStackByArgs(TimeStr, timeStr)
		}
...
func (d *Datum) convertToMysqlDecimal(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	if dec.negative && mysql.HasUnsignedFlag(target.Flag) {
		*dec = zeroMyDecimal
		if err == nil {
			err = tidb.ErrOverflow.GenWithStackByArgs("DECIMAL", fmt.Sprintf("(%d, %d)", target.Flen, target.Decimal))
		}
	}
...
func ProduceDecWithSpecifiedTp(dec *MyDecimal, tp *FieldType, sc *stmtctx.StatementContext) (_ *MyDecimal, err error) {
	...
	if flen != UnspecifiedLength && decimal != UnspecifiedLength {
		if flen < decimal {
			return nil, tidb.ErrMBiggerThanD.GenWithStackByArgs("")
		}
		...
		if !dec.IsZero() && prec-frac > flen-decimal {
			...
			err = tidb.ErrOverflow.GenWithStackByArgs("DECIMAL", fmt.Sprintf("(%d, %d)", flen, decimal))
			...
			if !dec.IsZero() && frac > decimal && dec.Compare(&old) != 0 {
				sc.AppendWarning(tidb.ErrTruncatedWrongVal.GenWithStackByArgs("DECIMAL", &old))
				err = nil
			}
		}
	}

	if tidb.ErrOverflow.Equal(err) {
...
func (d *Datum) convertToMysqlBit(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	if target.Flen <= 0 || target.Flen >= 128 {
		return Datum{}, errors.Trace(tidb.ErrDataTooLong.GenWithStack("Data Too Long, field len %d", target.Flen))
	}
	if target.Flen < 64 && uintValue >= 1<<(uint64(target.Flen)) {
		uintValue &= (1 << (uint64(target.Flen))) - 1
		err = tidb.ErrDataTooLong.GenWithStack("Data Too Long, field len %d", target.Flen)
	}
...
func (d *Datum) convertToMysqlEnum(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	if err != nil {
		err = errors.Wrap(tidb.ErrTruncated, "convert to MySQL enum failed: "+err.Error())
	}
...
func (d *Datum) convertToMysqlSet(sc *stmtctx.StatementContext, target *FieldType) (Datum, error) {
	...
	if err != nil {
		err = errors.Wrap(tidb.ErrTruncated, "convert to MySQL set failed: "+err.Error())
	}
...
func ChangeReverseResultByUpperLowerBound(
	...
	if terror.ErrorEqual(err, tidb.ErrOverflow) {
		return d, nil
	}
	if err != nil {
		return d, err
