...
func encode(sc *stmtctx.StatementContext, b []byte, vals []types.Datum, comparable bool) (_ []byte, err error) {
	...
	for i, length := 0, len(vals); i < length; i++ {
		...
		case types.KindMysqlDecimal:
			...
			} else if terror.ErrorEqual(err, tidb.ErrOverflow) {
...
func DecodeRange(b []byte, size int, idxColumnTypes []byte, loc *time.Location) ([]types.Datum, []byte, error) {
	if len(b) < 1 {
		return nil, b, errors.Error("invalid encoded key: length of key is zero")
	}
	...
	for len(b) > 1 {
		...
		if idxColumnTypes == nil {
			...
			if i >= len(idxColumnTypes) {
				return values, b, errors.Error("invalid length of index's columns")
			}
...
func DecodeOne(b []byte) (remain []byte, d types.Datum, err error) {
	if len(b) < 1 {
		return nil, d, errors.Error("invalid encoded key")
	}
	flag := b[0]
	b = b[1:]
	switch flag {
...
func peek(b []byte) (length int, err error) {
	originLength := len(b)
	if len(b) < 1 {
		return 0, errors.Error("invalid encoded key")
	}
	flag := b[0]
	length++
	b = b[1:]
...
func peekBytes(b []byte) (int, error) {
	...
	for {
		if len(b) < offset+encGroupSize+1 {
			return 0, errors.Error("insufficient bytes to decode value")
		}
...
func peekCompactBytes(b []byte) (int, error) {
	...
	vi := int(v)
	if n < 0 {
		return 0, errors.Error("value larger than 64 bits")
	} else if n == 0 {
		return 0, errors.Error("insufficient bytes to decode value")
	}
	if len(b) < vi+n {
