...
type Optimizer struct {
	...
	implementationRuleMap     map[Operand][]ImplementationRule
}

func NewPullQueryOptimizer() *Optimizer {
	return &Optimizer{
		transformationRuleBatches: PullQueryBatch,
...
func NewPushQueryOptimizer() *Optimizer {
	return &Optimizer{
		transformationRuleBatches: PushQueryBatch,
		implementationRuleMap:     defaultImplementationMap,
	}
}
...
func (opt *Optimizer) GetImplementationRules(node LogicalPlan) []ImplementationRule {
	return opt.implementationRuleMap[GetOperand(node)]
}
...
func (opt *Optimizer) FindBestPlan(sctx sessionctx.Context, logical LogicalPlan) (p PhysicalPlan, cost float64, err error) {
	logical, err = opt.onPhasePreprocessing(logical)
	if err != nil {
		return nil, 0, err
	}
	rootGroup := Convert2Group(logical)
	err = opt.onPhaseExploration(rootGroup)
	if err != nil {
		return nil, 0, err
	}
...
func (opt *Optimizer) onPhasePreprocessing(plan LogicalPlan) (LogicalPlan, error) {
...
func (opt *Optimizer) onPhaseExploration(g *Group) error {
...
func (opt *Optimizer) exploreGroup(g *Group, round int, ruleBatch TransformationRuleBatch) error {
	if g.Explored(round) {
		return nil
	}
	g.SetExplored(round)
...
func (opt *Optimizer) findMoreEquiv(g *Group, elem *list.Element, round int, ruleBatch TransformationRuleBatch) (eraseCur bool, err error) {
	expr := elem.Value.(*GroupExpr)
	operand := GetOperand(expr.ExprNode)
...
func (opt *Optimizer) fillGroupStats(g *Group) (err error) {
	if g.Prop.Stats != nil {
		return nil
	}
...
func (opt *Optimizer) onPhaseImplementation(sctx sessionctx.Context, g *Group) (PhysicalPlan, float64, error) {
	prop := &property.PhysicalProperty{
		ExpectedCnt: math.MaxFloat64,
	}
	preparePossibleProperties(g, make(map[*Group][][]*expression.Column))
	...
	if impl == nil {
		return nil, 0, tidb.ErrInternal.GenWithStackByArgs("Can't find a proper physical plan for this query")
	}
...
func (opt *Optimizer) implGroup(g *Group, reqPhysProp *property.PhysicalProperty, costLimit float64) (Implementation, error) {
...
func (opt *Optimizer) implGroupExpr(cur *GroupExpr, reqPhysProp *property.PhysicalProperty) (impls []Implementation, err error) {
...
// and LogicalJoin.
func preparePossibleProperties(g *Group, propertyMap map[*Group][][]*expression.Column) [][]*expression.Column {
