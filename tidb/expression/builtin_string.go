...
func (c *concatFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	maxAllowedPacket := ctx.GetSessionVars().MaxAllowedPacket
	if err != nil {
...
func (b *builtinConcatSig) evalString(row chunk.Row) (d string, isNull bool, err error) {
	...
	for _, a := range b.getArgs() {
		...
		if uint64(len(s)+len(d)) > b.maxAllowedPacket {
			b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("concat", b.maxAllowedPacket))
			return "", true, nil
		}
		s = append(s, []byte(d)...)
	}
...
func (b *builtinConcatWSSig) evalString(row chunk.Row) (string, bool, error) {
	...
	for i := 1; i < N; i++ {
		...
		if uint64(targetLength) > b.maxAllowedPacket {
			b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("concat_ws", b.maxAllowedPacket))
			return "", true, nil
		}
...
func (b *builtinRepeatSig) evalString(row chunk.Row) (d string, isNull bool, err error) {
	...
	if uint64(byteLength)*uint64(num) > b.maxAllowedPacket {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("repeat", b.maxAllowedPacket))
		return "", true, nil
	}
...
func (b *builtinSpaceSig) evalString(row chunk.Row) (d string, isNull bool, err error) {
	...
	if uint64(x) > b.maxAllowedPacket {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("space", b.maxAllowedPacket))
		return d, true, nil
	}
...
func (c *convertFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	if !ok {
		...
		return nil, tidb.ErrIncorrectArgs.GenWithStackByArgs("charset")
	}
	...
	if err != nil {
		return nil, tidb.ErrUnknownCharacterSet.GenWithStackByArgs(transcodingName)
	}
...
func (b *builtinConvertSig) evalString(row chunk.Row) (string, bool, error) {
	...
	// return with error.
	if encoding == nil {
		return "", true, tidb.ErrUnknownCharacterSet.GenWithStackByArgs(b.tp.Charset)
	}
...
func (b *builtinLpadSig) evalString(row chunk.Row) (string, bool, error) {
	...
	if uint64(targetLength) > b.maxAllowedPacket {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("lpad", b.maxAllowedPacket))
		return "", true, nil
	}
...
func (b *builtinRpadSig) evalString(row chunk.Row) (string, bool, error) {
	...
	targetLength := int(length)
	if uint64(targetLength) > b.maxAllowedPacket {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("rpad", b.maxAllowedPacket))
		return "", true, nil
	}
...
func (c *charFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	if _, ok := args[len(args)-1].(*Constant); !ok {
		...
		return nil, tidb.ErrIncorrectArgs
	}
...
func (b *builtinFormatWithLocaleSig) evalString(row chunk.Row) (string, bool, error) {
	...
	if isNull {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrUnknownLocale.GenWithStackByArgs("NULL"))
	} else if !strings.EqualFold(locale, "en_US") { // TODO: support other locales.
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrUnknownLocale.GenWithStackByArgs(locale))
	}
	locale = "en_US"
...
func (b *builtinFromBase64Sig) evalString(row chunk.Row) (string, bool, error) {
	...
	if needDecodeLen > int(b.maxAllowedPacket) {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("from_base64", b.maxAllowedPacket))
		return "", true, nil
	}
...
func (b *builtinToBase64Sig) evalString(row chunk.Row) (d string, isNull bool, err error) {
	...
	if needEncodeLen > int(b.maxAllowedPacket) {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("to_base64", b.maxAllowedPacket))
		return "", true, nil
	}
...
func (b *builtinInsertSig) evalString(row chunk.Row) (string, bool, error) {
	...
	if uint64(strLength-length+int64(len(newstr))) > b.maxAllowedPacket {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("insert", b.maxAllowedPacket))
		return "", true, nil
	}
...
func (c *loadFileFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	return nil, tidb.ErrFunctionNotExists.GenWithStackByArgs("FUNCTION", "load_file")
}
...
func (c *weightStringFunctionClass) verifyArgs(args []Expression) (weightStringPadding, int, error) {
	...
	l := len(args)
	if l != 1 && l != 3 {
		return weightStringPaddingNone, 0, tidb.ErrIncorrectParameterCount.GenWithStackByArgs(c.funcName)
	}
	...
	if l == 3 {
		if args[1].GetType().EvalType() != types.ETString {
			return weightStringPaddingNone, 0, tidb.ErrIncorrectType.GenWithStackByArgs(args[1].String(), c.funcName)
		}
		...
		default:
			return weightStringPaddingNone, 0, tidb.ErrIncorrectType.GenWithStackByArgs(x, c.funcName)
		}
		if args[2].GetType().EvalType() != types.ETInt {
			return weightStringPaddingNone, 0, tidb.ErrIncorrectType.GenWithStackByArgs(args[2].String(), c.funcName)
		}
...
func (b *builtinWeightStringSig) evalString(row chunk.Row) (string, bool, error) {
	...
	case weightStringPaddingAsChar:
		...
		if b.length < lenRunes {
			...
			if uint64(b.length-lenRunes) > b.maxAllowedPacket {
				b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("weight_string", b.maxAllowedPacket))
				return "", true, nil
			}
	...
	case weightStringPaddingAsBinary:
		...
		if b.length < lenStr {
			...
			b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrTruncatedWrongValue.GenWithStackByArgs(tpInfo, str))
			str = str[:b.length]
		} else if b.length > lenStr {
			if uint64(b.length-lenStr) > b.maxAllowedPacket {
				b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrWarnAllowedPacketOverflowed.GenWithStackByArgs("cast_as_binary", b.maxAllowedPacket))
				return "", true, nil
			}
	...
	case weightStringPaddingNone:
		ctor = collate.GetCollator(b.args[0].GetType().Collate)
	default:
		return "", false, tidb.ErrIncorrectType.GenWithStackByArgs(ast.WeightString, string(b.padding))
	}
