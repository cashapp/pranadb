...
func (b *builtinAbsIntSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if val == math.MinInt64 {
		return 0, false, tidb.ErrOverflow.GenWithStackByArgs("BIGINT", fmt.Sprintf("abs(%d)", val))
	}
	return -val, false, nil
}
...
func (b *builtinLog1ArgSig) evalReal(row chunk.Row) (float64, bool, error) {
	...
	if val <= 0 {
		b.ctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrInvalidArgumentForLogarithm)
		return 0, true, nil
	}
...
func (b *builtinPowSig) evalReal(row chunk.Row) (float64, bool, error) {
	...
	if math.IsInf(power, -1) || math.IsInf(power, 1) || math.IsNaN(power) {
		return 0, false, tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", fmt.Sprintf("pow(%s, %s)", strconv.FormatFloat(x, 'f', -1, 64), strconv.FormatFloat(y, 'f', -1, 64)))
	}
...
func (b *builtinConvSig) conv(str string, fromBase, toBase int64) (res string, isNull bool, err error) {
	...
	if err != nil {
		return res, false, tidb.ErrOverflow.GenWithStackByArgs("BIGINT UNSINGED", str)
	}
	if signed {
...
func (b *builtinCotSig) evalReal(row chunk.Row) (float64, bool, error) {
	...
	return 0, false, tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", fmt.Sprintf("cot(%s)", strconv.FormatFloat(val, 'f', -1, 64)))
}
...
func (b *builtinExpSig) evalReal(row chunk.Row) (float64, bool, error) {
	...
	if math.IsInf(exp, 0) || math.IsNaN(exp) {
		...
		return 0, false, tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", s)
	}
...
func (c *truncateFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	default:
		return nil, tidb.ErrIncorrectArgs.GenWithStackByArgs("truncate")
	}
