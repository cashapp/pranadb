...
func (b *builtinDateSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if expr.IsZero() {
		return types.ZeroTime, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, expr.String()))
	}
...
func (c *dateLiteralFunctionClass) getFunction(ctx sessionctx.Context, args []Expression) (builtinFunc, error) {
	...
	if !datePattern.MatchString(str) {
		return nil, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, str)
	}
...
func (b *builtinDateLiteralSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if mode.HasNoZeroDateMode() && b.literal.IsZero() {
		return b.literal, true, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, b.literal.String())
	}
...
func (b *builtinDateDiffSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if invalidLHS, invalidRHS := lhs.InvalidZero(), rhs.InvalidZero(); invalidLHS || invalidRHS {
		if invalidLHS {
			err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, lhs.String()))
		}
		if invalidRHS {
			err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, rhs.String()))
		}
		return 0, true, err
	}
...
func calculateTimeDiff(sc *stmtctx.StatementContext, lhs, rhs types.Time) (d types.Duration, isNull bool, err error) {
	...
	if tidb.ErrTruncatedWrongVal.Equal(err) {
...
func (b *builtinDateFormatSig) evalString(row chunk.Row) (string, bool, error) {
	...
	if t.InvalidZero() {
		...
		return "", true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
...
func (b *builtinMonthNameSig) evalString(row chunk.Row) (string, bool, error) {
	...
	if (arg.IsZero() && b.ctx.GetSessionVars().SQLMode.HasNoZeroDateMode()) || mon < 0 || mon > len(types.MonthNames) {
		return "", true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
...
func (b *builtinDayNameSig) evalIndex(row chunk.Row) (int64, bool, error) {
	...
	if arg.InvalidZero() {
		return 0, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
	}
...
func (b *builtinWeekWithModeSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if date.IsZero() {
		return 0, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, date.String()))
	}
...
func (b *builtinWeekWithoutModeSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	mode := 0
	modeStr := b.ctx.GetSessionVars().DefaultWeekFormat
	if modeStr != "" {
		mode, err = strconv.Atoi(modeStr)
		if err != nil {
			return 0, true, handleInvalidTimeError(b.ctx, tidb.ErrInvalidWeekModeFormat.GenWithStackByArgs(modeStr))
		}
	}
...
func evalFromUnixTime(ctx sessionctx.Context, fsp int8, unixTimeStamp *types.MyDecimal) (res types.Time, isNull bool, err error) {
	...
	if err != nil && !terror.ErrorEqual(err, tidb.ErrTruncated) {
		return res, true, err
	}
...
func (b *builtinStrToDateDateSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if !succ {
		return types.ZeroTime, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
	if b.ctx.GetSessionVars().SQLMode.HasNoZeroDateMode() && (t.Year() == 0 || t.Month() == 0 || t.Day() == 0) {
		return types.ZeroTime, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValueForType.GenWithStackByArgs(types.DateTimeStr, date, ast.StrToDate))
	}
...
func (b *builtinStrToDateDurationSig) evalDuration(row chunk.Row) (types.Duration, bool, error) {
	...
	succ := t.StrToDate(sc, date, format)
	if !succ {
		return types.Duration{}, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
	}
	t.SetFsp(int8(b.tp.Decimal))
...
func (b *builtinExtractDatetimeFromStringSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	dt, err := types.ParseDatetime(sc, dtStr)
	if err != nil {
		if !terror.ErrorEqual(err, tidb.ErrWrongValue) {
			return 0, true, err
		}
	}
	if dt.IsZero() {
...
func (du *baseDateArithmetical) getDateFromString(ctx sessionctx.Context, args []Expression, row chunk.Row, unit string) (types.Time, bool, error) {
	...
	if err != nil {
		...
		return types.ZeroTime, true, handleInvalidTimeError(ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, dateStr))
	}
...
func (du *baseDateArithmetical) addDate(ctx sessionctx.Context, date types.Time, year, month, day, nano int64) (types.Time, bool, error) {
	...
	if goTime.Year() < 0 || goTime.Year() > 9999 {
		return types.ZeroTime, true, handleInvalidTimeError(ctx, tidb.ErrDatetimeFunctionOverflow.GenWithStackByArgs("datetime"))
	}
...
func (du *baseDateArithmetical) vecGetDateFromString(b *baseBuiltinFunc, input *chunk.Chunk, unit string, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if err != nil {
			...
			return handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, dateStr))
		} else {
			dates[i] = date
		}
...
func (b *builtinUnixTimestampCurrentSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if !terror.ErrorEqual(err, tidb.ErrTruncated) {
		terror.Log(err)
	}
...
func (b *builtinUnixTimestampIntSig) evalIntWithCtx(ctx sessionctx.Context, row chunk.Row) (int64, bool, error) {
	...
	if err != nil && terror.ErrorEqual(tidb.ErrWrongValue.GenWithStackByArgs(types.TimeStr, val), err) {
...
func (b *builtinAddDatetimeAndStringSig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if err != nil {
		if terror.ErrorEqual(err, tidb.ErrTruncatedWrongVal) {
			sc.AppendWarning(err)
			return types.ZeroDatetime, true, nil
		}
...
func (b *builtinMakeDateSig) evalTime(row chunk.Row) (d types.Time, isNull bool, err error) {
	...
	if retTimestamp == 0 {
		return d, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, startTime.String()))
	}
...
func (b *builtinPeriodAddSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if !validPeriod(p) {
		return 0, false, tidb.ErrIncorrectArgs.GenWithStackByArgs("period_add")
	}
...
func (b *builtinPeriodDiffSig) evalInt(row chunk.Row) (int64, bool, error) {
	...
	if !validPeriod(p1) {
		return 0, false, tidb.ErrIncorrectArgs.GenWithStackByArgs("period_diff")
	}
...
func (b *builtinSecToTimeSig) evalDuration(row chunk.Row) (types.Duration, bool, error) {
	...
	if hour > 838 {
		...
		second = 59
		demical = 0
		err = b.ctx.GetSessionVars().StmtCtx.HandleTruncate(tidb.ErrTruncatedWrongValue.GenWithStackByArgs("time", strconv.FormatFloat(secondsFloat, 'f', -1, 64)))
		if err != nil {
...
func (b *builtinTimestampAddSig) evalString(row chunk.Row) (string, bool, error) {
	...
	default:
		return "", true, tidb.ErrWrongValue.GenWithStackByArgs(types.TimeStr, unit)
	}
...
func (b *builtinLastDaySig) evalTime(row chunk.Row) (types.Time, bool, error) {
	...
	if tm.Month() == 0 || (tm.Day() == 0 && b.ctx.GetSessionVars().SQLMode.HasNoZeroDateMode()) {
		return types.ZeroTime, true, handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String()))
	}
...
func handleInvalidZeroTime(ctx sessionctx.Context, t types.Time) (bool, error) {
	...
	return true, handleInvalidTimeError(ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String()))
}
