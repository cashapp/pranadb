...
func (b *builtinDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(times); i++ {
		...
		if times[i].IsZero() {
			if err := handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, times[i].String())); err != nil {
				return err
			}
...
func (b *builtinDayNameSig) vecEvalIndex(input *chunk.Chunk, apply func(i, res int), applyNull func(i int)) error {
	...
	for i := 0; i < n; i++ {
		...
		if ds[i].InvalidZero() {
			if err := handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, ds[i].String())); err != nil {
				return err
			}
			applyNull(i)
			continue
...
func (b *builtinYearWeekWithoutModeSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		date := ds[i]
		if date.InvalidZero() {
			if err := handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, date.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinPeriodDiffSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if !validPeriod(i64s[i]) || !validPeriod(periods[i]) {
			return tidb.ErrIncorrectArgs.GenWithStackByArgs("period_diff")
		}
...
func (b *builtinStrToDateDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if !succ {
			if err := handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinToSecondsSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if ret == 0 {
			if err := handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, arg.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinMakeDateSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if retTimestamp == 0 {
			if err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, startTime.String())); err != nil {
				return err
			}
			result.SetNull(i, true)
			continue
...
func (b *builtinWeekWithoutModeSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	if modeStr != "" {
		...
		if err != nil {
			return handleInvalidTimeError(b.ctx, tidb.ErrInvalidWeekModeFormat.GenWithStackByArgs(modeStr))
		}
	}
	for i := 0; i < n; i++ {
		if result.IsNull(i) {
...
func (b *builtinPeriodAddSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if !validPeriod(i64s[i]) {
			return tidb.ErrIncorrectArgs.GenWithStackByArgs("period_add")
		}
...
func (b *builtinTimestampAddSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		default:
			return tidb.ErrWrongValue.GenWithStackByArgs(types.TimeStr, unit)
		}
...
func (b *builtinDateFormatSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := range times {
		...
		if t.InvalidZero() {
			...
			if errHandled := handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, t.String())); errHandled != nil {
				return errHandled
			}
			continue
		}
...
func (b *builtinDateDiffSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if invalidArg0, invalidArg1 := args0[i].InvalidZero(), args1[i].InvalidZero(); invalidArg0 || invalidArg1 {
			if invalidArg0 {
				err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, args0[i].String()))
			}
			if invalidArg1 {
				err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, args1[i].String()))
			}
			if err != nil {
				return err
			}
...
func (b *builtinTimestampDiffSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if invalidLHS, invalidRHS := lhs[i].InvalidZero(), rhs[i].InvalidZero(); invalidLHS || invalidRHS {
			if invalidLHS {
				err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, lhs[i].String()))
			}
			if invalidRHS {
				err = handleInvalidTimeError(b.ctx, tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, rhs[i].String()))
			}
			if err != nil {
				return err
			}
...
func (b *builtinDateLiteralSig) vecEvalTime(input *chunk.Chunk, result *chunk.Column) error {
	...
	if mode.HasNoZeroDateMode() && b.literal.IsZero() {
		return tidb.ErrWrongValue.GenWithStackByArgs(types.DateTimeStr, b.literal.String())
	}
