...
import (
	...
	math2 "github.com/squareup/pranadb/tidb/util/math"
)
...
func (b *builtinArithmeticMultiplyRealSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		x[i] = x[i] * y[i]
		if math.IsInf(x[i], 0) {
			return tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", fmt.Sprintf("(%s * %s)", b.args[0].String(), b.args[1].String()))
		}
	}
	return nil
}
...
func (b *builtinArithmeticDivideDecimalSig) vecEvalDecimal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if err == tidb.ErrDivByZero {
			...
			if err = sc.HandleTruncate(tidb.ErrTruncatedWrongValue.GenWithStackByArgs("DECIMAL", to)); err != nil {
				return err
			}
		} else if err == nil {
...
func (b *builtinArithmeticMinusIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(lhi64s); i++ {
		...
		if overflow {
			return tidb.ErrOverflow.GenWithStackByArgs(errType, fmt.Sprintf("(%s - %s)", b.args[0].String(), b.args[1].String()))
		}
...
func (b *builtinArithmeticIntDivideDecimalSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if err == tidb.ErrTruncated {
			err = sc.HandleTruncate(tidb.ErrTruncatedWrongValue.GenWithStackByArgs("DECIMAL", c))
		} else if err == tidb.ErrOverflow {
			newErr := tidb.ErrTruncatedWrongValue.GenWithStackByArgs("DECIMAL", c)
		...
		if isUnsigned {
			...
			if err == tidb.ErrOverflow {
				v, err := c.ToInt()
				// when the final result is at (-1, 0], it should be return 0 instead of the error
				if v == 0 && err == tidb.ErrTruncated {
					i64s[i] = int64(0)
					continue
				}
				return tidb.ErrOverflow.GenWithStackByArgs("BIGINT UNSIGNED", fmt.Sprintf("(%s DIV %s)", num[0][i].String(), num[1][i].String()))
			}
			i64s[i] = int64(val)
		} else {
			i64s[i], err = c.ToInt()
			...
			if err == tidb.ErrOverflow {
				return tidb.ErrOverflow.GenWithStackByArgs("BIGINT", fmt.Sprintf("(%s DIV %s)", num[0][i].String(), num[1][i].String()))
			}
		}
	}
	return nil
...
func (b *builtinArithmeticMultiplyIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if (x[i] != 0 && tmp/x[i] != y[i]) || (tmp == math.MinInt64 && x[i] == -1) {
			result.SetNull(i, true)
			return tidb.ErrOverflow.GenWithStackByArgs("BIGINT", fmt.Sprintf("(%s * %s)", b.args[0].String(), b.args[1].String()))
		}
...
func (b *builtinArithmeticPlusIntSig) plusUU(result *chunk.Column, lhi64s, rhi64s, resulti64s []int64) error {
	for i := 0; i < len(lhi64s); i++ {
		...
		if uint64(lh) > math.MaxUint64-uint64(rh) {
			return tidb.ErrOverflow.GenWithStackByArgs("BIGINT UNSIGNED", fmt.Sprintf("(%s + %s)", b.args[0].String(), b.args[1].String()))
		}
