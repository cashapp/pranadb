...
func (b *builtinConvertSig) vecEvalString(input *chunk.Chunk, result *chunk.Column) error {
	...
	// return with error.
	if encoding == nil {
		return tidb.ErrUnknownCharacterSet.GenWithStackByArgs(b.tp.Charset)
	}
	result.ReserveString(n)
	for i := 0; i < n; i++ {
		if expr.IsNull(i) {
...
func formatDecimal(sctx sessionctx.Context, xBuf *chunk.Column, dInt64s []int64, result *chunk.Column, localeBuf *chunk.Column) error {
	...
	for i := range xDecimals {
		...
		if localeBuf == nil {
			...
			// FORMAT(x, d, NULL)
			sctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrUnknownLocale.GenWithStackByArgs("NULL"))
		} else if !strings.EqualFold(localeBuf.GetString(i), "en_US") {
			// TODO: support other locales.
			sctx.GetSessionVars().StmtCtx.AppendWarning(tidb.ErrUnknownLocale.GenWithStackByArgs(localeBuf.GetString(i)))
		}
