...
func (b *builtinCotSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(f64s); i++ {
		...
		if err := tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", fmt.Sprintf("cot(%s)", strconv.FormatFloat(f64s[i], 'f', -1, 64))); err != nil {
			return err
		}
	}
	return nil
...
func (b *builtinExpSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(f64s); i++ {
		...
		if math.IsInf(exp, 0) || math.IsNaN(exp) {
			...
			if err := tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", s); err != nil {
				return err
			}
		}
		f64s[i] = exp
...
func (b *builtinPowSig) vecEvalReal(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if math.IsInf(power, -1) || math.IsInf(power, 1) || math.IsNaN(power) {
			return tidb.ErrOverflow.GenWithStackByArgs("DOUBLE", fmt.Sprintf("pow(%s, %s)", strconv.FormatFloat(x[i], 'f', -1, 64), strconv.FormatFloat(y[i], 'f', -1, 64)))
		}
		f64s[i] = power
	}
	return nil
...
func (b *builtinAbsIntSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < len(i64s); i++ {
		...
		if i64s[i] == math.MinInt64 {
			return tidb.ErrOverflow.GenWithStackByArgs("BIGINT", fmt.Sprintf("abs(%d)", i64s[i]))
		}
		if i64s[i] < 0 {
			i64s[i] = -i64s[i]
		}
