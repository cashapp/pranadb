	...
	if len(b.args) == 3 {
		...
		for i := 0; i < nr; i++ {
			...
			if pathExpr.ContainsAnyAsterisk() {
				return tidb.ErrInvalidJSONPathWildcard
			}
	...
	for i := 0; i < nr; i++ {
		...
		if escapeBuf != nil && !escapeBuf.IsNull(i) {
			...
			if len(escapeStr) == 0 {
				...
				return tidb.ErrIncorrectArgs.GenWithStackByArgs("ESCAPE")
			}
		}
		...
		if pathBufs != nil {
			...
			for j := 0; j < len(b.args)-4; j++ {
				...
				if err != nil {
					return tidb.ErrInvalidJSONPath.GenWithStackByArgs(pathBufs[j].GetString(i))
				}
				pathExprs = append(pathExprs, pathExpr)
			}
		}
...
func (b *builtinJSONArrayInsertSig) vecEvalJSON(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < nr; i++ {
		...
		for j := 0; j < (len(b.args)-1)/2; j++ {
			...
			if pathExpr.ContainsAnyAsterisk() {
				return tidb.ErrInvalidJSONPathWildcard.GenWithStackByArgs(pathBufs[j].GetString(i))
			}
...
func (b *builtinJSONContainsPathSig) vecEvalInt(input *chunk.Chunk, result *chunk.Column) error {
	...
	for i := 0; i < n; i++ {
		...
		if containType != json.ContainsPathAll && containType != json.ContainsPathOne {
			return tidb.ErrInvalidJSONContainsPathType
		}
